...
class LLMEngine:
    ...
    def _initialize_kv_caches(self) -> None:
        ...
        # TODO fix this...
        if self.model_config.hf_config.model_type in ["deepseek_v3", "deepseek_v2", "deepseek2", "deepseek3"] and self.model_config.quantization == "gguf":
            from vllm.distributed import get_tensor_model_parallel_world_size
            assert get_tensor_model_parallel_world_size() == 1, print("Combining CPU inference and GGUF quantization for deepseek model with multi-gpu is not supported for now!")
            num_gpu_blocks = int(num_gpu_blocks * 0.6)
            num_cpu_blocks = int(num_cpu_blocks * 0.6)
