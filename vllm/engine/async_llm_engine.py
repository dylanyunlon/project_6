    ...
    async def run_engine_loop(engine_ref: ReferenceType):
        ...
        while True:
            if not any(has_requests_in_progress):
                ...
                # may be we could do not use this when on server mode(all vllm server in new process)
                # why i change the official code to the following code:
                # when use pp, vllm will create "pipeline_parallel_size" tasks to get full parallel inference.
                # assume we use pp 4, then requests_in_progress contains 4 taks, the question is:
                # asyncio.wait(tasks, asyncio.FIRST_COMPLETED) will return immediately if there is no input to process on engine ve.
                # so, a lots of asyncio.sleep(0) will be executed.
                # By change the code to the following, we only execute the engine which has inputs, and the new inputs and it's task 
                # will be created and executed when first taks completed.
            ...
            try:
                async with asyncio_timeout(ENGINE_ITERATION_TIMEOUT_S):
                    ...
                    for _ in range(len(requests_in_progress)):
                        await asyncio.sleep(0)
                add_virtual_engine = set(list(range(num_virtual_engine)))
                for task in done:
                    result = task.result()
