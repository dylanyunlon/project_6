...
class MLACommonMetadataBuilder(Generic[M]):
    ...
    def _build_decode(self, input_positions: torch.Tensor,
                      ...
                      max_decode_seq_len: int, use_cuda_graph: bool):
    ...
    def build(self, num_reqs: int, num_actual_tokens: int, max_query_len: int,
        ...
        if self._num_decodes > 0:
            decode_metadata = self._build_decode(
                ...
                max_decode_seq_len=max_query_len,
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def forward(
        ...
        if attn_metadata is None:
            # Profiling run.
            output = torch.empty(output.shape[0], self.o_proj.output_size,device=hidden_states_or_q_c.device,
        ...
        if has_decode:
            ...
            decode_q_pe, decode_k_pe = self.rotary_emb(attn_metadata.decode.input_positions, decode_q_pe, decode_k_pe)
        ...
        if has_prefill:
            ...
            ixf_ops.mla_rope(attn_metadata.prefill.input_positions, prefill_q_pe, prefill_k_pe.squeeze(1), prefill_k[...,self.qk_nope_head_dim:], self.rotary_emb.cos_sin_cache)
