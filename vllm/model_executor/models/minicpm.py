...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_scale = self.config.scale_depth / math.sqrt(self.config.num_hidden_layers)
...
class MiniCPMModel(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.k = None
        if self.layers[self.start_layer].self_attn.rotary_emb.__class__.__name__ == "Phi3LongRoPEScaledRotaryEmbedding":
            self.k = self.layers[self.start_layer].self_attn.rotary_emb.original_max_position_embeddings
    ...
    def forward(
        ...
        if self.k is not None:
            long_offset = torch.any(positions > self.k)
        else:
            long_offset = None
        ...
        hidden_states, _ = self.norm(residual, hidden_states, self.layers[self.start_layer].hidden_scale)
...
class MiniCPMForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        for _, layer in self.model.named_modules():
            if layer.__class__.__name__ == "MiniCPM3Attention":
                dtype = layer.kv_a_proj_with_mqa.weight.dtype
                if dtype not in [torch.half, torch.bfloat16, torch.int8]:
                    assert False, print(f"run on dtype:{dtype} is not supported.")
