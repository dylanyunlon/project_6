...
class GGUFModelLoader(BaseModelLoader):
    ...
    def _prepare_weights(self, model_name_or_path: str):
        if os.path.isfile(model_name_or_path):
            ...
            guff_files = glob.glob(os.path.join(model_name_or_path, "*.gguf"))
            if not guff_files:
                raise ValueError(
                    f"Could not find checkpoint files *.gguf', only gguf files are currently supported!")
            return guff_files
    ...
    def load_model(self, vllm_config: VllmConfig) -> nn.Module:
        ...
        gguf_extra_tensor_names = []
        local_model_path = [os.path.join(model_config.model, x) for x in local_model_path]
        for file in local_model_path:
            gguf_extra_tensor_names.extend(get_gguf_extra_tensor_names(file, gguf_weights_map))
        # we can only know if tie word embeddings after mapping weights
        if "lm_head.weight" in gguf_extra_tensor_names:
        ...
        with set_default_torch_dtype(model_config.dtype):
            ...
            all_tensors = []
            all_reader = []
            for gguf_file in local_model_path:
                reader = gguf.GGUFReader(gguf_file)
                all_tensors.extend(reader.tensors)
                all_reader.append(reader)
            
            def all_gguf_weights():
                for tensor in all_tensors:
                    if tensor.name in gguf_weights_map:
                        weight_type = tensor.tensor_type
                        name = gguf_weights_map[tensor.name]
            ...
            model.load_weights(all_gguf_weights())
            
            setattr(model, "reader_for_ref", all_reader)
            ...
            for _, module in tqdm(model.named_modules()):
                ...
                if quant_method is not None and hasattr(quant_method, "process_weights_after_loading"):
                    if module.__class__.__name__ == "FusedMoE":
                        quant_method.process_weights_after_loading(module, all_tensors, gguf_weights_map)
                    else:
                        quant_method.process_weights_after_loading(module)
