...
class BertSelfOutput(nn.Module):
    ...
    def forward(
        ...
        hidden_states = ops.linear(hidden_states, self.dense.weight, self.dense.bias)
        hidden_states = ops.bert_add_norm(
            input=hidden_states,
            residual=input_tensor,
            ln_weight=self.LayerNorm.weight,
            ln_bias=self.LayerNorm.bias,
            epsilon=self.eps,
        )
...
class BertAttention(nn.Module):
    ...
    def forward(
        ...
        enable_unpad: Optional[bool] = False,
        max_seq_len: Optional[int] = None,
    ) -> Tuple[torch.Tensor]:
        if self.qkv_weight is None:
            self.qkv_weight = torch.cat(
                [self.self.query.weight, self.self.key.weight, self.self.value.weight],
                dim=0,
            )

            self.qkv_bias = torch.cat(
                [self.self.query.bias, self.self.key.bias, self.self.value.bias], dim=0
            )
            del self.self.query
            del self.self.key
            del self.self.value
        qkv = ops.linear(hidden_states, self.qkv_weight, self.qkv_bias)
        q, k, v = torch.chunk(qkv, 3, dim=-1)

        num_heads = self.self.num_attention_heads
        head_size = self.self.attention_head_size

        if not enable_unpad:

            bs, seq_len, hidden_size = q.shape

            q = (
                q.view(bs, seq_len, num_heads, head_size)
                .permute(0, 2, 1, 3)
                .contiguous()
            )
            k = (
                k.view(bs, seq_len, num_heads, head_size)
                .permute(0, 2, 1, 3)
                .contiguous()
            )
            v = (
                v.view(bs, seq_len, num_heads, head_size)
                .permute(0, 2, 1, 3)
                .contiguous()
            )

            self_outputs = ops.scaled_dot_product_attention(
                q,
                k,
                v,
                attn_mask=attention_mask,
            ...
            k = k.view(num_tokens, num_heads, head_size)
            v = v.view(num_tokens, num_heads, head_size)
            self_outputs = ops.ixinfer_flash_attn_unpad(
                q,
                k,
                v,
                cu_seq_lens,
                ...
                max_seq_len,
                is_causal=False,
                atten_scale=1 / math.sqrt(head_size),
            )
            self_outputs = (self_outputs,)
            attention_output = self_outputs[0].view(num_tokens, hidden_size)
...
class BertIntermediate(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ori_shape = list(hidden_states.shape)
        ori_shape[-1] = self.dense.weight.shape[0]

        hidden_states = ops.act_bias_mm(
            mat1=hidden_states.view(-1, hidden_states.shape[-1]),
            mat2=self.dense.weight,
            bias=self.dense.bias,
            act_type="gelu",
            trans_format="TN",
        )
        return hidden_states.view(*ori_shape)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.pooler is not None and enable_unpad:
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        logits = ops.linear(
            sequence_output, self.qa_outputs.weight, self.qa_outputs.bias
        )

        if enable_unpad:
            start_logits, end_logits = ops.bert_unpack_start_end_logits(
                logits, cu_seq_lens, max_seq_len
            )
        else:
