...
class CrossAttnUpBlock2D(nn.Module):
    ...
    def forward(
        ...
        if self.upsamplers is not None:
            for upsampler in self.upsamplers:                
                if not hidden_states.requires_grad and hidden_states.shape[0]>=64:
                    _chunk_size=8
                    num_chunks = hidden_states.shape[0] // _chunk_size
                    hidden_states = torch.cat(
                        [
                            upsampler(hid_slice, upsample_size, scale=lora_scale)

                            for hid_slice in hidden_states.chunk(num_chunks, dim=0)
                        ],
                        dim=0,
                    )
                else:
        ...
        for idx, resnet in enumerate(self.resnets):
            ...
            if self.training and self.gradient_checkpointing:
                ...
                if not hidden_states.requires_grad and hidden_states.shape[0]>=64:
                    ...
                    hidden_states = torch.cat(
                        [
                            resnet(hid_slice, temb_slice, scale=scale)

                            for hid_slice,temb_slice in zip(hidden_states.chunk(num_chunks, dim=0),temb.chunk(num_chunks, dim=0))
                        ],
                        dim=0,
                    )
                else:
        ...
        if self.upsamplers is not None:
            for upsampler in self.upsamplers:
                if not hidden_states.requires_grad and hidden_states.shape[0]>=64:
                    ...
                    hidden_states = torch.cat(
                        [
                            upsampler(hid_slice, upsample_size, scale=scale)
