...
from .modeling_codeshell_ixformer import mha, mlp_forward
...
class CodeShellAttention(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.kv_heads = (
            config.num_query_groups if self.group_query_attention else self.num_heads
        )
    ...
    def _init_rope(self):
        if self.rope_scaling is None:
            self.rotary_emb = CodeShellRotaryEmbedding(
    ...
    def forward(
        ...
        key_states = key_states.view(
            bsz, q_len, self.num_query_groups, self.head_dim
        ).transpose(1, 2)
        ...
        if using_pytorch:
            ...
            if attention_mask is not None:
                ...
                mask_value = self._get_mask_value(
        ...
        if using_ixformer:
            # print(f"query_states.dtype {query_states.dtype, key_states.dtype,value_states.dtype}")
            attn_output = mha(query_states, key_states, value_states, attention_mask)
        # print(attn_output.shape,attn_output_ixformer.shape)
        # diff = attn_output-attn_output_ixformer
        # print(f"attention_mask {attention_mask.dtype,attention_mask.shape}")
        # print(f"diff {diff.max()} attention_mask is None {attention_mask is None}")
...
class CodeShellMLP(nn.Module):
    ...
    def forward(self, hidden_states: Optional[Tuple[torch.Tensor]]) -> torch.Tensor:
        using_pytorch = False
        using_ixformer = True
        # hidden_states_ixf = hidden_states.clone()
        if using_pytorch:
        ...
        if using_ixformer:
            hidden_states = mlp_forward(self, hidden_states)
        # diff = (hidden_states-hidden_states_ixf)
        # print(f"mlp diff {diff.max()}")
        return hidden_states
...
class CodeShellModel(CodeShellPreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [
                CodeShellBlock(config, layer_idx=i)
    ...
    def forward(
        ...
        if attention_mask is not None:
            self_attention_mask = self_attention_mask * attention_mask.reshape(
                batch_size, 1, -1
...
class CodeShellForCausalLM(CodeShellPreTrainedModel):
    ...
    def chat(
        ...
        input_ids = self.build_chat_input(
            query, history, tokenizer, generation_config.max_new_tokens
        )
        stopping_criteria = StoppingCriteriaList(
            [
                EndOfFunctionCriteria(
                    [len(input_ids[0])],
                    ["|<end>|", "|end|", "<|endoftext|>", "## human"],
                    tokenizer,
                )
            ]
        )
    ...
    def generate_stream(self, prompt, tokenizer, generation_config=None, **kwargs):
        generation_config = generation_config or self.generation_config
        max_input_tokens = (
            self.config.n_positions - self.generation_config.max_new_tokens
        )
