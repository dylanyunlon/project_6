...
class PunicaWrapperGPU(PunicaWrapperBase):
    ...
    def _apply_expand(
        ...
        if self.is_prefill:
            if self.no_lora:
                return y
            ops.sbgmv_expand(x, w_t_all, y[:, y_offset:y_offset+y_slice_size], *self.prefill_metadata, add_input=add_inputs)
        else:
            ops.sbgmv_expand(x, w_t_all, y[:, y_offset:y_offset+y_slice_size], lora_indices_tensor=self.token_lora_indices, add_input=add_inputs)
    def add_expand(self,
                   y: torch.Tensor,
                   x: torch.Tensor,
