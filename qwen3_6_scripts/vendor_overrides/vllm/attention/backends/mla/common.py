...
class MLACommonImpl(MLAAttentionImpl[T], Generic[T]):
    ...
    def _v_up_proj_and_o_proj(self, x):
        ...
        return self.o_proj(x.reshape(-1, self.num_heads * self.v_head_dim))[0]
    ...
    def _q_proj_and_k_up_proj(self, x):
        ...
        return torch.einsum("bnp,lnp->bnl", q_nope, self.W_UK).view(-1, self.num_heads, self.kv_lora_rank), q_pe
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if envs.VLLM_MLA_CUSTOMIZE:
            ...
            if hasattr(layer, "scheme") and layer.scheme.__class__.__name__ == "CompressedTensorsW8A8Int8":
                # must be as same as kv_b_proj
                assert hasattr(self.q_proj, "scheme") and self.q_proj.scheme.__class__.__name__ == "CompressedTensorsW8A8Int8"
                ...
                def _v_up_proj_and_o_proj_w8a8(self, x):
                    ...
                    return self.o_proj(res.reshape(-1, self.num_heads * self.v_head_dim))[0]

                def _q_proj_and_k_up_proj_w8a8(self, x):
                    # x: b, q_lora
                    x = self.q_proj(x)[0].view(-1, self.num_heads, self.qk_head_dim)
                    x_nope, x_pe = x.split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
                    # W_UK = (self.W_UK * self.W_UK_S.view(1, self.num_heads, self.qk_nope_head_dim)).to(x.dtype)
                    # return torch.einsum("bnp,lnp->bnl", x_nope, W_UK).view(-1, self.num_heads, self.kv_lora_rank), x_pe
                    res = ixf_ops.marlin_w8a16(
                        x_nope,
                        self.W_UK,
                        self.W_UK_S,
                ...
                self._v_up_proj_and_o_proj = functools.partial(_v_up_proj_and_o_proj_w8a8, self)
                self._q_proj_and_k_up_proj = functools.partial(_q_proj_and_k_up_proj_w8a8, self)
                ...
                self.w4_group_size = self.q_proj.quant_method.quant_config.group_size
                ...
                def _v_up_proj_and_o_proj_w4a16(self, x):
                    # W16A16
                    # res = torch.einsum("bnl,lnv->bnv", x, self.W_UV_fp16)

                    # W4A16
                    res = ixf_ops.marlin_w4a16(
                        x, self.W_UV, self.W_UV_S, self.W_UV_Z,
                        group_size=self.w4_group_size,
                        format="k16n32",
                        batch_first=False)

                    return self.o_proj(res.view(-1, self.num_heads * self.v_head_dim))[0]

                def _q_proj_and_k_up_proj_w4a16(self, x):
                    x = self.q_proj(x)[0].view(-1, self.num_heads, self.qk_head_dim)
                    x_nope, x_pe = x.split([self.qk_nope_head_dim, self.qk_rope_head_dim], dim=-1)
    ...
    def _forward_prefill(
        ...
        #                                    value=0)
        # attn_output = torch.empty(q.shape[0], self.num_heads, q.shape[-1], dtype=q.dtype, device=q.device)
        ...
        # output = output.view(-1, self.num_heads, q.shape[-1])[..., :v.shape[-1]].reshape(-1, self.num_heads * self.v_head_dim)
    ...
    def forward(
        ...
        if has_decode:
            ...
            decode_q_pe, decode_k_pe = self.rotary_emb(decode_input_positions, decode_q_pe, decode_k_pe)
        ...
        if has_prefill:
            ...
            ixf_ops.mla_rope(prefill_input_positions, prefill_q_pe, prefill_k_pe.squeeze(1), prefill_k[...,self.qk_nope_head_dim:], self.rotary_emb.cos_sin_cache)
        ...
        if kv_cache.numel() > 0:
            ...
            if has_decode:
                if envs.VLLM_USE_INT8_MLA:
                    write_kv_cache = (None, None)
                    k_c_normed_int8, k_c_normed_scale,_ = ops.scaled_int8_quant(k_c_normed[num_prefill_tokens:])
                    decode_k_pe_int8, decode_k_pe_scale,_ = ops.scaled_int8_quant(decode_k_pe)
                    ops.concat_and_cache_mla_int8(
                        ...
                        slot_mapping = attn_metadata.slot_mapping.flatten()[num_prefill_tokens:],
                ...
                #         decode_k_pe,
                #         kv_cache,
                #         attn_metadata.slot_mapping.flatten()[num_prefill_tokens:],
                #         kv_cache_dtype=self.kv_cache_dtype,
                #         scale=layer._k_scale,
                #     )
                    write_kv_cache = (k_c_normed[num_prefill_tokens:], decode_k_pe)
            if has_prefill:
                if envs.VLLM_USE_INT8_MLA:
        ...
        if has_prefill:
            if 0 == attn_metadata.num_decode_tokens:
                return self._forward_prefill(
                    prefill_q, prefill_k_c_normed, prefill_k, kv_cache, kv_cache_scale,
                    attn_metadata)
        ...
        if has_decode:
            if attn_metadata.num_prefill_tokens == 0:
                return self._forward_decode(
                    decode_ql_nope, decode_q_pe, kv_cache, kv_cache_scale, attn_metadata, *write_kv_cache)
