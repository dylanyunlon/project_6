...
class ChatCompletionRequest(OpenAIBaseModel):
    ...
    def normalize_messages(cls, data):
        ...
        if system_messages:
            ...
            if all(isinstance(c, str) for c in system_contents):
                merged_system = {
                    **system_messages[0],
    ...
    def validate_bi100_prompt_logprob_sample(cls, data):
        ...
        if (not isinstance(positions, list)
                ...
                or any(
                    not isinstance(p, int) or isinstance(p, bool)
                    or p <= 0 or p >= 262144 for p in positions)
